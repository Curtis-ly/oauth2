...
// Token 令牌信息
type Token struct {
	ClientID         string        `bson:"ClientID" orm:"pk;column(client_id)"`    // 客户端标识
	UserID           string        `bson:"UserID" orm:"column(user_id)"`           // 用户标识
	RedirectURI      string        `bson:"RedirectURI" orm:"column(redirect_url)"` // 重定向URI
